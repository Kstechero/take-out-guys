...
- 响应方式：SSE 流式响应
- 当前范围：管理端 SSE 对话、模型健康检查、管理端 Tool Calling、用户端 Tool Calling、管理端本地知识检索与只读 MCP 风格能力目录
- 尚未实现：会话持久化、Embedding、向量检索、用户侧正式知识库和推荐闭环
...
- 推理服务：GX10 vLLM
- 对话模型：`ornith`
- 当前能力：管理端 SSE 流式响应、模型健康检查、管理端与用户端 Tool Calling、管理端基于 LangChain4j 文档抽象的本地知识检索、只读 MCP 风格能力目录
- 安全约束：实时业务事实优先通过工具读取；变更类工具需要显式确认；当前 MCP bridge 仅开放只读能力
- 规划能力：会话持久化、用户侧正式知识库、Embedding、向量检索、智能推荐、内容生成与外部化 MCP Server
...
4. AI 服务层  
   当前包括管理端 SSE 对话、模型健康检查、管理端与用户端 Tool Calling、管理端本地知识检索和只读 MCP 风格能力目录；向量化检索、会话持久化与用户侧正式知识库为规划能力。
...
AI 智能客服不是简单的问答功能，而是结合了多轮对话、知识检索和工具调用能力。当前已落地的是管理端与用户端共享的业务知识检索链路，以及管理端/用户端业务 Tool Calling。对于平台规则类问题，系统优先通过知识检索回答；对于订单类问题，系统调用订单相关接口获取真实订单数据；对于复杂售后和投诉类问题，系统引导用户转接人工客服。
...
- 每个会话最多保留 20 条历史消息；
- 使用 SSE 实现流式响应；
- 当前管理端已提供本地文档检索能力，后续再升级为 embedding + 向量检索；
...
## 十、RAG 知识库规划（向量化增强尚未实现）

当前代码已经在管理端与用户端落地基于 LangChain4j 文档抽象的共享业务知识检索，用于读取 `docs/` 和数据库设计文档等静态知识源；本节描述的是下一阶段的向量化 RAG 方案，而不是当前已上线的轻量知识检索实现。若未来继续升级，可将平台规则、配送范围、售后说明、优惠券规则和堂食规则整理为正式知识库文档，再根据选定的 Embedding 模型与向量数据库完成检索增强。
...
The current AI scope covers both the admin operations assistant and the first complete user-side AI loop. The codebase now includes persistent AI sessions, intelligent recommendation, review drafting, sensitive-word moderation, and human customer service CRUD APIs, while embeddings and advanced knowledge retrieval remain roadmap items.
