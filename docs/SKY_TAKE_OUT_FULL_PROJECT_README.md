# Takeout Guys 智能外卖平台完整项目说明

## 一、项目介绍

**Takeout Guys** 是一个面向消费者与餐饮运营人员的智能外卖服务平台。项目基于基础版苍穹外卖系统进行二次开发，原系统已经具备用户端点餐、购物车、下单、订单管理、菜品管理、套餐管理、员工管理和后台运营管理等基础能力。Takeout Guys 在此基础上重构了前端体验，并进一步拓展业务功能与智能化能力。
...
在 AI 能力方面，当前后端通过 OpenAI 兼容协议连接 GX10 vLLM 服务，使用 `ornith` 模型实现管理端 AI 助手，并通过 SSE 返回流式内容。目前已经实现管理端流式对话与模型健康检查；用户端 AI、业务工具调用、会话记忆、Embedding 和 RAG 知识库仍属于后续规划，尚未在当前代码中落地。
...
**基于 Spring Boot、Vue 3、uni-app 与大模型能力构建的一体化智能外卖服务平台。**

项目核心目标包括：

1. 对原有苍穹外卖项目前端进行重构优化；
2. 补充评价、优惠券、堂食、人工客服、个人中心等业务模块；
3. 完善大语言模型能力，逐步实现用户端 AI 助手、智能推荐和 AI 帮写评价；
4. 评估通过 RAG 技术提升平台规则、配送范围和售后政策问答准确性的可行性；
5. 规划知识检索与向量存储方案；
6. 规划业务工具调用，让 AI 助手未来能够结合真实业务数据回答；
...
相比基础版苍穹外卖系统，以下内容同时包含已完成改造与后续产品规划，具体交付状态以代码和 API 文档为准：
...
- 开发框架：Spring Boot 2.7.3
- ORM 框架：MyBatis
- 缓存框架：Spring Cache
- 数据库：MySQL 8.0
- 缓存数据库：Redis
- AI 接入：原生 Java HTTP 客户端调用 OpenAI 兼容 API
- 推理服务：GX10 vLLM
- 当前模型：`ornith`（可通过 `GX10_AI_MODEL` 配置）
- 响应方式：SSE 流式响应
- 当前范围：管理端 AI 对话与模型健康检查
- 尚未实现：Embedding、RAG、向量检索和业务工具调用
...
当前 AI 能力聚焦管理端运营助手；用户端智能客服、智能推荐、内容生成和知识库检索属于后续规划。
...
- 推理服务：GX10 vLLM
- 对话模型：`ornith`
- 当前能力：管理端单轮运营问答、SSE 流式响应、模型健康检查
- 安全约束：未接入业务工具时明确禁止编造订单、营业额、库存和退款数据
- 规划能力：用户端 AI、会话管理、智能推荐、内容生成、业务工具调用与知识库检索
...
4. AI 服务层  
   当前包括管理端 SSE 对话和健康检查；用户端 AI、RAG、向量化、会话记忆和工具调用为规划能力。

5. 数据存储层  
   包括 MySQL、Redis 缓存和文件存储；当前未使用 Redis 向量库。
...
本节是可选的后续技术方案，并非当前实现。若未来引入 RAG，可将平台规则、配送范围、售后说明、优惠券规则和堂食规则整理为知识库文档，再根据选定的 Embedding 模型与向量数据库完成检索增强。
...
2. 通过 OpenAI 兼容协议接入 GX10 vLLM 的 `ornith` 模型；
3. 实现管理端 AI 运营助手的 SSE 流式对话与健康检查；
4. 为 AI 设置业务数据防编造约束；
5. 保留用户端 AI、RAG 和业务工具调用的扩展空间；
...
# Takeout Guys — Complete Intelligent Food Delivery Platform Guide

## 1. Project Introduction

**Takeout Guys** is an intelligent food delivery platform for customers and restaurant operators. It is built as a secondary development of the basic Sky Take-Out system, which already provides core capabilities such as ordering, shopping cart management, order placement, dish and set meal management, employee management, and backend operations. Takeout Guys builds on that foundation with a redesigned frontend, expanded business workflows, and AI-assisted experiences.
...
For AI capabilities, the backend currently connects to a GX10 vLLM service through an OpenAI-compatible API and uses the `ornith` model for the admin assistant. The implemented scope consists of admin SSE streaming chat, model health checks, and admin-side LLM tool calling against real business services. `AdminAiChatServiceImpl` now keeps only the LLM dialogue plus tool-calling path: legacy fixed replies, keyword-triggered branching, and the old upstream direct-streaming path have been removed. Admin tools return structured JSON instead of preformatted natural-language summaries, and `query_orders`, `query_coupons`, and `query_goods` support `all=true` with backend fetch-all pagination so the model can inspect complete datasets instead of only the default first page. User-side conversation memory, embeddings, and RAG remain roadmap items and are not implemented in the current codebase.
...
Takeout Guys is positioned as:

**An integrated intelligent food delivery platform built with Spring Boot, Vue 3, uni-app, and large-model capabilities.**
...
4. Evaluate RAG for future platform-rule, delivery-scope, and after-sales Q&A;
5. Plan an appropriate knowledge retrieval and vector storage solution;
...
The following list combines completed changes and roadmap items. Code and official API documents are the source of truth for delivery status:
...
- AI integration: native Java HTTP client using an OpenAI-compatible API
- Inference service: GX10 vLLM
- Current model: `ornith` (configurable through `GX10_AI_MODEL`)
- Response mode: SSE streaming
- Current scope: admin AI chat, model health checks, and admin-side business tool calling with structured JSON tool outputs and fetch-all support for list queries
- Not implemented: embeddings, RAG, vector retrieval, and user-side conversation memory
...
The current AI scope focuses on the admin operations assistant. User-side AI, intelligent recommendation, content generation, and knowledge retrieval are roadmap items.

The AI technology stack includes:

- Protocol: OpenAI-compatible API
- Inference service: GX10 vLLM
- Dialogue model: `ornith`
- Current capabilities: admin SSE streaming, model health checks, and LLM tool calling for shop status, order operations, coupon lookup, goods lookup, business overview, and trend analysis; the service keeps only the LLM + tool-calling path, and list tools can expose complete datasets through `all=true`
- Safety constraint: current business data and business actions must go through registered tools instead of free-form model guesses
- Planned capabilities: user-side AI, conversation management, recommendation, content generation, and knowledge retrieval
...
4. AI service layer  
   The current implementation provides admin SSE chat, health checks, and admin-side tool calling. Tool results are returned as structured JSON so the model can reason over raw business data directly, including full-result reads for supported list tools. User-side AI, RAG, embeddings, and conversation memory are planned.

5. Data storage layer  
   This includes MySQL, Redis cache, and file storage. Redis is not currently used as a vector store.
...
1. Integrate a configurable dialogue model;
...
- Uses a unified AI service abstraction to avoid provider lock-in;
...
- Based on a configurable dialogue model;
- Uses a unified AI service abstraction for model integration;
...
- Selects an embedding model according to deployment requirements;
- Selects a vector database according to data scale;
...
- Uses a configurable dialogue model to understand dish information and user sentiment;
...
## 10. RAG Knowledge Base Plan (Not Implemented)

This section describes an optional future architecture, not the current implementation. A future RAG solution could organize platform rules and business rules as knowledge documents, then use a selected embedding model and vector database for retrieval augmentation.
...
Selected embedding model
        ↓
Stored in the selected vector database
        ↓
...
Prompt context construction
        ↓
Dialogue model generates a response with retrieved context
        ↓
...
- Select an embedding model according to deployment requirements;
- Select a vector database according to data scale;
...
Recommendation prompt is constructed
        ↓
The configured dialogue model is called
        ↓
...
User enters simple review requirements
        ↓
The configured dialogue model generates the review
        ↓
...
2. GX10 vLLM `ornith` integration through an OpenAI-compatible API;
3. Admin AI assistant with SSE streaming, health checks, and business tool calling;
4. Explicit safeguards against fabricated business data;
5. Extensible architecture for future user-side AI, RAG, and business tools;
...
Takeout Guys combines a frontend refactoring of the basic Sky Take-Out system with an initial AI integration. It preserves core ordering and operational capabilities and currently provides an admin AI operations assistant. Intelligent recommendation, AI-assisted review writing, human customer service, coupons, dine-in ordering, and RAG remain roadmap items.
