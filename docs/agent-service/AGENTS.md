...
- 权威后端目录是 `backend/`；`backend-legacy/` 只保留为历史迁移基线，禁止承接新业务或作为运行部署来源。
- 权威项目仓库：`https://github.com/Kstechero/take-out-guys`。
- Agent 微服务必须在独立功能分支开发，分支统一以 `codex/` 开头；禁止直接在 `main` 实现或合并未验证的 Agent 改动。
- 当前 Java AI 实现是迁移基线。用户侧对话直接使用 Agent API；管理端迁移完成前保留旧实现源码，不再新增用户侧 `legacy` 运行时分支。
...
以下 `backend-legacy/` 文件仅用于理解历史 Java AI 基线，不是当前实现或部署目标：

- `backend-legacy/sky-server/src/main/java/com/sky/service/ai/AiToolCallingClient.java`：现有 OpenAI 兼容 Tool Calling 循环；
- `backend-legacy/sky-server/src/main/java/com/sky/service/ai/user/UserAiToolRegistry.java` 与 `UserAiToolExecutor.java`：用户侧工具定义和业务分发；
- `backend-legacy/sky-server/src/main/java/com/sky/service/ai/admin/AdminAiToolRegistry.java` 与 `AdminAiToolExecutor.java`：管理侧工具定义和业务分发；
- `backend-legacy/sky-server/src/main/java/com/sky/service/ai/knowledge/OperationsKnowledgeService.java`：现有轻量关键词知识检索；
- `backend-legacy/sky-server/src/main/java/com/sky/service/impl/UserAiChatServiceImpl.java` 与 `AdminAiChatServiceImpl.java`：现有 AI 入口与 SSE 行为。

Python 服务的职责是替换“模型编排、RAG、工具选择”能力，不是复制业务逻辑。原 Executor 中对订单、购物车、地址、优惠券、评价和敏感词的真实业务调用，应逐步收敛为 Spring Boot Internal Agent API。

## 架构边界

- 本服务使用 Python、FastAPI、LangChain 和 LangGraph，保持无状态 HTTP 部署；用户和管理图分别使用独立状态与 checkpointer namespace。
...
- Agent Service 失败、模型不可用或向量库故障时，必须返回可理解的降级结果；生产恢复使用标准部署版本回滚。
...
- 实现前阅读 `docs/agent-service/LANGCHAIN_RAG_AGENT_MICROSERVICE_PLAN.md` 和本目录下的契约、工具、RAG、提示词、测试、ADR 文档。
...
- 不修改 `backend-legacy/`、`admin-web/`、`user-app/`，除非当前任务明确授权且接口契约已经确认。
- 管理端迁移完成前不删除其依赖的 `AiToolCallingClient`、Registry、Executor；现有 `/user/ai/**`、`/admin/ai/**` 外部路由保持兼容。
