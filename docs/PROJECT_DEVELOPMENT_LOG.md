...
### 2026-07-07 · Admin Review Moderation and User Review Display

- Scope: admin frontend, user mini-program, backend review APIs, admin AI tools, and docs.
- Delivered a real admin review-management flow: added `/admin/review/page`, `/admin/review/{id}/status/{status}`, and `/admin/review/{id}`; the Vue 3 admin console now renders a dedicated review page with keyword filtering, status filtering, detail viewing, hide/restore actions, and delete actions.
- Completed the user-side review display loop in the mini-program: dish detail popups now show a real review preview, and a new `pages/dish-reviews/index` page loads paginated review lists with image preview and like toggling.
- Extended review persistence and moderation support: added admin/user review page DTOs and VOs, exposed current-user review paging, made review deletion cascade through `dish_review_like`, and expanded review-status payloads with partial/full review progress fields.
- Added sensitive-word management to the admin AI tool layer so the operations assistant can query, create, update, and delete sensitive words through confirmed tool calls.
- Files: `admin-web/src/views/ReviewsView.vue`, `admin-web/src/api/admin.ts`, `admin-web/src/router/index.ts`, `backend/sky-server/src/main/java/com/sky/controller/admin/AdminReviewController.java`, `backend/sky-server/src/main/java/com/sky/service/impl/ReviewServiceImpl.java`, related review DTO/VO/mapper files, `user-app/pages/dish-reviews/index.vue`, `user-app/pages/index/index.vue`, `user-app/pages/index/index.js`, `user-app/pages/index/style.scss`, `user-app/pages/api/api.js`, `user-app/pages.json`, `backend/sky-server/src/main/java/com/sky/service/ai/admin/AdminAiToolExecutor.java`, `backend/sky-server/src/main/java/com/sky/service/ai/admin/AdminAiToolRegistry.java`, and docs.
- Validation: `cd admin-web && npm.cmd run build`; Vue SFC parsing passed for the changed mini-program pages via `@vue/compiler-sfc`.
